...
class Owner(commands.Cog, name="Owner"):
    ...
    async def sync_cmds(self, ctx, scope: str = "guild"):
        ...
        try:
            if scope.lower() == "global":
                ...
                view = make_text_container(f"{emojis.CHECK_ICO} Cleared all guild-specific slash commands for this server! Duplicate commands will disappear shortly.")
                ...
                for g in self.bot.guilds:
                    try:
                        ...
                        pass
                view = make_text_container(f"{emojis.CHECK_ICO} Cleared guild-specific slash commands for `{cleared_count}` server(s) successfully!")
            else:
    ...
    async def setactivity(self, ctx, *, text: str):
        if text.lower() == "auto":
            ...
            view = make_text_container(f"{emojis.CHECK_ICO} Auto presence rotation enabled.")
        else:
            ...
            view = make_text_container(f"{emojis.CHECK_ICO} Activity set to `{text}`. Auto presence rotation disabled (use `>setactivity auto` to enable it).")
        await ctx.reply(view=view)
    ...
    async def toggle_maintenance(self, ctx, state: str = "check"):
        ...
        if state in ("on", "enable", "1", "true"):
            ...
            await self.bot.db.log_audit_action(
                ...
                title="🛠️ Web Maintenance Mode ENABLED",
                description="Web dashboard disabled for public users via Discord command.",
                color=0xef4444
            )
            view = make_text_container(
                f"### {emojis.WARNING} Web Dashboard Maintenance: **ENABLED**\n"
                f"{emojis.DOT} Web player and public dashboard access are now **OFF** for users.\n"
                f"{emojis.DOT} Owners can still access the Admin Panel (`/admin`)."
            )
            ...
            await self.bot.db.log_audit_action(
                user_id=ctx.author.id,
                username=str(ctx.author),
                title="🟢 Web Maintenance Mode DISABLED",
                description="Web dashboard enabled for all users via Discord command.",
                color=0x10b981
            )
            view = make_text_container(
                f"### {emojis.SUCCESS} Web Dashboard Maintenance: **DISABLED**\n"
                f"{emojis.DOT} Web player and public dashboard access are now **ONLINE** for all users."
            )
        else:
            is_active = await self.bot.db.get_bot_setting("maintenance_mode", "0") == "1"
            status_str = "**ENABLED (OFF)** 🛠️" if is_active else "**DISABLED (ONLINE)** 🟢"
            view = make_text_container(
                f"### {emojis.INFO} Web Dashboard Status\n"
                f"{emojis.DOT} Current Status: {status_str}\n"
                f"{emojis.DOT} Usage: `{ctx.prefix}maintenance on` | `{ctx.prefix}maintenance off`"
            )
        await ctx.reply(view=view)
